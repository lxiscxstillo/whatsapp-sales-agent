...
const QR_VALIDITY_SECS = 20;       // WPPConnect rotates QR every ~20s
...
function QrPanel({
  ...
  // Countdown resets only when the QR image itself changes (every ~20s), not on each poll.
  // Use first 60 chars of qrcode as a stable key to detect actual QR rotation.
  const qrKey = data.qrcode?.slice(0, 60) ?? '';
  const [secs, setSecs] = useState(QR_VALIDITY_SECS);

  useEffect(() => {
    setSecs(QR_VALIDITY_SECS);
  ...
  return (
    ...
    >
      ...
      {/* Tip */}
      <div className="text-xs text-slate-500 bg-slate-50 rounded-xl px-4 py-2.5 mb-4 text-center leading-relaxed">
        Abre WhatsApp &gt; <strong>Dispositivos vinculados</strong> &gt; <strong>Vincular dispositivo</strong>, luego escanea este código inmediatamente.
      </div>
      ...
      <div className="flex flex-col items-center gap-4 mb-6">
        ...
        {/* Progress bar — counts down 20s from when THIS QR was generated */}
        <div className="w-64 space-y-1.5">
          <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-1000 ease-linear ${secs <= 5 ? 'bg-red-400' : secs <= 10 ? 'bg-amber-400' : 'bg-emerald-500'}`}
              style={{ width: `${(secs / QR_VALIDITY_SECS) * 100}%` }}
            />
          </div>
          <div className="flex items-center justify-center gap-1.5 text-xs text-slate-400">
            ...
            <span>{secs <= 0 ? 'Actualizando QR…' : `Código válido por ${secs}s — ¡escanea ahora!`}</span>
          </div>
        </div>
